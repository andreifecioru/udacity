...
public class MainActivity extends AppCompatActivity implements
    ...
    protected void onResume() {
        ...
        showCharging(checkChargingStatus());
    }

    private boolean checkChargingStatus() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            BatteryManager batteryManager = (BatteryManager) getSystemService(BATTERY_SERVICE);
            boolean status = batteryManager != null && batteryManager.isCharging();
            Log.d(LOG_TAG, "Charging status: " + status);
            return status;
        }

        // Pre API-23 compatible code (sticky intents)
        IntentFilter intentFilter = new IntentFilter(Intent.ACTION_BATTERY_CHANGED);
        // pass null as receiver param to create a sticky intent
